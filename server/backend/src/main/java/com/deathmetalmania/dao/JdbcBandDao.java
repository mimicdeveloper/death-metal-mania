...
public class JdbcBandDao implements BandDao {
    ...
    @Override
    public void add(Band band) {  // Changed to use Band entity instead of BandDto
    ...
    @Override
    public void updateBand(int id, Band band) {  // Changed to use Band entity instead of BandDto
        String sql = "UPDATE bands SET name = ?, genre = ?, country = ? " +
                "WHERE band_id = ?";
        try {
            jdbcTemplate.update(sql, band.getName(), band.getGenre(), band.getCountry(), id);
        } catch (Exception e) {
